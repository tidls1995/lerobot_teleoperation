...
- **호밍 중 그리퍼를 움직여야 하는가.** 지금은 `home_pose` 의 그리퍼 값으로 이동한다.
  거의 닫힌 값이면 쥐고 있던 것을 조이고, 열린 값이면 떨어뜨린다. **호밍에서 그리퍼를
  아예 건드리지 않는 것**이 제안된 해법이다 — 쥔 것을 유지하고, 그리퍼는 리더가 쉽게
  도달할 수 있어 정렬에도 문제가 없다. 미결정.
- **풋페달** — 양팔 작업에 필수. 기종 미선정.
